...
class ValueModel(tf.keras.Model):
    def __init__(self, mode=0, debug=None, input_dim_str='2', job_cols=1, hidden_dim_str='1000,250', node_module=None, window_size=5):  # ?
        ...
        self.conv1d_layer = tf.keras.layers.Conv1D(
            1, self.input_dim[1], input_shape=(self.input_dim[1], 1))
        # this is our dense hidden layer witha ReLU activiation that will encode most of our information
        self.hidden_layer1 = tf.keras.layers.Dense(
            self.hidden_dim[0], 'relu', input_shape=(self.input_dim[0],), use_bias=False)
        self.hidden_layer2 = tf.keras.layers.Dense(
            self.hidden_dim[1], 'relu', input_shape=(self.hidden_dim[0],), use_bias=False)
...
class ForwardAgent():
    ...
    def learning_rate_update(self):
        lr = self.lr_init * tf.math.pow(self.decay, tf.math.floordiv(
            tf.cast(self.episode_count, tf.float32), self.period))
